...
prompt_path = "tests/scenarios/checkout-architecture-execution.prompt.md"
expected_path = "tests/scenarios/checkout-architecture-execution.expected.yaml"
actual_path = "tests/scenarios/checkout-architecture-execution.actual.yaml"

prompt = File.read(prompt_path)
raise "prompt does not invoke skill" unless prompt.include?("$orchestrating-architecture-execution")
raise "prompt does not enforce human-sized scope" unless prompt.include?("human-sized")
raise "prompt does not stop before coding" unless prompt.include?("Stop before coding")
